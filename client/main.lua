...
local function takeCoral(coral)
  if PlayerData.job.type == "leo" or PlayerData.job.type == "fire" then QBCore.Functions.Notify(Lang:t("error.job_check"), "error") return end
...
RegisterNetEvent('qb-diving:client:NewLocations', function()
    QBCore.Functions.TriggerCallback('qb-diving:server:GetDivingConfig', function(config, area)
        Config.CoralLocations = config
        if PlayerData.job.type ~= "leo" or PlayerData.job.type ~= "fire" then setDivingLocation(area) end
        --setDivingLocation(area)
    end)
end)
...
RegisterNetEvent('qb-diving:client:CallCops', function(coords, msg)
    ...
    QBCore.Functions.Notify(Lang:t("error.police_notification", "error", msg))
    --TriggerEvent("chatMessage", Lang:t("error.police_notification"), "error", msg)
    local transG = 100
